 ...
 * @param {string} [options.label] - Optional label shown above the bubble (e.g. "Generated by Gemini")
 */
export function drawSpeechBubble(ctx, options) {
  ...
  const padding = 10;
  const fontSize = isCode ? 14 : 13;
  const lineHeight = isCode ? 18 : 17;
  const tailH = 10;
  ...
  // Draw label above bubble (e.g. "Generated by Gemini")
  if (label) {
    ...
    ctx.fillText(label, bubbleX + 2, bubbleY - 4);
  }
